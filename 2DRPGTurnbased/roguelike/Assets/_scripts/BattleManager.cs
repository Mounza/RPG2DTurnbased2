...
public class BattleManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            BattleStart(new string[] { "Blob" }, false);
        }
        ...
        if(!battleActive)
        {
            ...
            for(int i = 0; i < playerPositions.Length; i++)
            {
                if(GameManager.instance.playerStats[i].gameObject.activeInHierarchy)
                {
                    ...
                    activeBattlers[i].intelligence = thePlayer.intelligence;
    ...
    public void DealDamage(int target, int movePower)
    {
        ...
        float mgcPwr = activeBattlers[currentTurn].intelligence;
        float defPwr = activeBattlers[target].defence + activeBattlers[target].armrPower;

        // float damageCalc = (atkPwr / defPwr) * movePower * Random.Range(.9f, 1.1f);
        float damageCalc = mgcPwr * movePower * Random.Range(.9f, 1.1f);
